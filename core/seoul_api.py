...
이 모듈은 Streamlit에 의존하지 않는다. 무거운 API 응답 캐싱은 core.cache 의
ttl_cache 로 처리하므로, 백엔드 API 등 다른 실행 환경에서도 그대로 재사용할 수
있다. Streamlit 진입점은 루트의 seoul_api.py shim 을 통해 이 모듈을 사용한다.
"""
...
@ttl_cache(CONGESTION_CACHE_TTL)
...
@ttl_cache(BUS_RIDERSHIP_CACHE_TTL)
...
@ttl_cache(SUBWAY_RIDERSHIP_CACHE_TTL)
